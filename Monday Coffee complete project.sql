...
ALTER TABLE city
ALTER COLUMN estimated_rent TYPE float;
...
with t1 as 
		(
			select 
				...
				group by ci.city_name,cu.customer_name, product			
		)
	select *,
			sum(t1.counts) over(partition by t1.city order by t1.name ) as rolling_total
		from t1
		where t1.product <= 14;
